...
- 优化认证组件的视觉层级、响应式安全区、语义 token、触控反馈和暗色模式表现，保持组件库默认风格克制且可主题化。
- 增强 `AuthVisualCarousel` 的自动播放控制、悬停与焦点暂停、减少动态效果适配、移动端信息密度和轮播辅助技术语义。
- 完善登录和注册表单的独立控件 ID、自动填充属性、字段级错误关联、首个错误字段聚焦和长内容处理。
- 统一用户名、密码、手机号和验证码输入框的 Lucide 前缀图标尺寸、占位和文本起始位置。
- 补充认证组件设计 Review Skill 约定、公开用法文档和 playground 示例。

### 修复

- 修复手机号输入区域被发送验证码按钮拉伸，导致输入框前缀图标视觉不一致的问题。
- 修复 `enabledMethods` 为空或运行时变化时登录表单可能没有可用方式的问题。
- 修复 `AuthPageShell` 自定义 `overlay` 被强制禁用指针事件或从辅助技术树隐藏的问题，同时保持默认视觉遮罩不可交互。

## [0.7.0] - 2026-06-30

### 破坏性变更

- `lucide-react` peer dependency 统一升级到 `^1.22.0`，消费方需要使用 `lucide-react` 1.x 版本。
